# 2026-04-30 Cancel Acropolis on Input Output Engineering Core Development Proposal (2of6)

## Transaction ID : ``

## Transaction Inputs

The UTxO marked `UTXO-EC-0002-25-02`

#### UTXO:`339e8a1b493d7b9e6895ea68d944829aa37a082191ffb5e6175896950d2fae04#0`

## Transaction Outputs

Milestones 10 through 14 have been cancelled, each with a budget of 289,714 ada, for a total of 1,448,570 ada. This amount should be returned to the treasury contract, while the remaining balance of 2,356,346 ada should be redirected back to the vendor contract.

### Treasury reserve contract

#### DESTINATION:`addr1xxzc8pt7fgf0lc0x7eq6z7z6puhsxmzktna7dluahrj6g6v9swzhujsjlls7dajp59u95re0qdk9vh8mumlemw89535s4ecqxj`

#### AMOUNT_LOVELACE:`1448570000000`

### Change for Vendor Contract

#### CHANGE_AMOUNT_LOVELACE:`2356346000000`

#### ADDRESS:`addr1xxyzewehw7dh78ea62mkgdnzmcdlcxqt4u39a7pqc0v0at5g9janwaum0u0nm54hvsmx9hsmlsvqhteztmuzps7cl6hq7d35th`

## Datum

Removal of milestones with identifiers : M-10, M-11, M-12, M-13, M-14

## Required Signatures

- Oversight Committee
  - Sundae Labs keyhash : 1880102b04725318eb7a6f9f481815c82473c2f50cfe9932c85a3bf8
  - Cardano Foundation keyhash : ac3de68e8e1f9af3e00b299e7858bc44e126b33e130d06223d799d5d
- Intersect Leadership
  - Leadership 2 keyhash: 91f5b1d436080c1beca93fbbb96596312d8f615b0ad9e94470af2224
- Intersect Admin
  - Admin 2 keyhash : a664de561ccd2ca9a07c060d4dd7cea4dc68ba89d4bf04b21ff0726f
  - Admin 3 keyhash : c236d8dc9441605238a5fe9cc7e6c1a79903289a4249c33122bf9627
- The vendor
  - Vendor payment keyhash : 33afc56ecef7fc370f59d5574416826d6bd3d1f88e0f449ed5ae79f5
  - Vendor wallet address : addr1qye6l3twemmlcdc0t824w3qksfkkh573lz8q73y76kh8na0ua8vtvcf4r53sgychpd696knp4suy8f3mwgyt7d33cjds499p2r

## Transaction Metadata

See [metadata.json](./metadata.json).